...
parse_package([Ignore|Package]) ->
  log4erl:debug(trace, "IGNORAD TRACE ~n ~p ~n~p~n", [Ignore, Package]),
...
parse_event('Dial', Elements) ->
  {Record, ActionID} = call_record(Elements, #call{}, nil),
  {parse_phones(Record),ActionID};

parse_event('Bridge', Elements) ->
  bridge_record(Elements, #bridge{}, nil);

parse_event('Rename', Elements) ->
  {#rename{channel = Channel, newname = Newname} = Record, ActionID} = rename_record(Elements, #rename{}, nil),
  {Record#rename{old_number = normalize_number(parse_sip_number(list_to_binary(Channel), "0")),
  new_number = normalize_number(parse_sip_number(list_to_binary(Newname), "0"))}, ActionID};
...
unknown_event([Binary|T], Acc, ActionID) ->
  ...
  case re:run(String, "[A-Za-z0-9]+: ") of
    {match, [{Start0, Len}|_]} ->
      Start = Start0 + 1,
    ...
    nomatch ->
      ?warning(io_lib:format("Not match ~p", [String]))
  end;
...
call_record([<<"SubEvent: ", SubEvent/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{subevent = list_to_atom(binary_to_list(SubEvent))}, ActionID);
call_record([<<"Channel: ", Channel/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{channel = binary_to_list(Channel)}, ActionID);
call_record([<<"Destination: ", Dest/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{destination = binary_to_list(Dest)}, ActionID);
call_record([<<"CallerIDNum: ", CID/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{caller_id_num = binary_to_list(CID)}, ActionID);
call_record([<<"CallerIDName: ", CIDName/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{caller_id_name = binary_to_list(CIDName)}, ActionID);
call_record([<<"UniqueID: ", UID/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{unique_id = binary_to_list(UID)}, ActionID);
call_record([<<"DestUniqueID: ", DestUID/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{dest_unique_id = binary_to_list(DestUID)}, ActionID);
call_record([<<"Dialstring: ", Dialstring/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{dialstring = binary_to_list(Dialstring)}, ActionID);
call_record([<<"DialStatus: ", DialStatus/binary>>|T], Record, ActionID) ->
  call_record(T, Record#call{dialstatus = binary_to_list(DialStatus)}, ActionID);
...
bridge_record([<<"Privilege: ", Priv/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{privilege = privileges_list(Priv)}, ActionID);
bridge_record([<<"Bridgestate: ", Bridgestate/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{bridgestate = binary_to_list(Bridgestate)}, ActionID);
bridge_record([<<"Bridgetype: ", Bridgetype/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{bridgetype = binary_to_list(Bridgetype)}, ActionID);
bridge_record([<<"Channel1: ", Channel1/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{channel_1 = binary_to_list(Channel1)}, ActionID);
bridge_record([<<"Channel2: ", Channel2/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{channel_2 = binary_to_list(Channel2)}, ActionID);
bridge_record([<<"Uniqueid1: ", Uniqueid1/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{unique_id_1 = binary_to_list(Uniqueid1)}, ActionID);
bridge_record([<<"Uniqueid2: ", Uniqueid2/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{unique_id_2 = binary_to_list(Uniqueid2)}, ActionID);
bridge_record([<<"CallerID1: ", CallerID1/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{caller_id_1 = binary_to_list(CallerID1)}, ActionID);
bridge_record([<<"CallerID2: ", CallerID2/binary>>|T], Record, ActionID) ->
  bridge_record(T, Record#bridge{caller_id_2 = binary_to_list(CallerID2)}, ActionID);
bridge_record([_Field|T], Record, ActionID) ->
  %?warning(io_lib:format("Ignoring ~p in call record.", [Field])),
  bridge_record(T, Record, ActionID);
bridge_record([], Record, ActionID) ->
  {Record, ActionID}.
...
originate_record([<<"Uniqueid: ", UID/binary>>|T], Record, ActionID) ->
  originate_record(T, Record#originate{unique_id = binary_to_list(UID)},
    ActionID);
originate_record([_Field|T], Record, ActionID) ->
...
voicemail_record([<<"Old: ", Old/binary>>|T], Record, ActionID) ->
  voicemail_record(T, Record#voicemail{old = binary_to_integer(Old)},
    ActionID);
voicemail_record([_Field|T], Record, ActionID) ->
...
queue_event([<<"Reason: ", Reason/binary>>|T], Record, ActionID) ->
  queue_event(T, Record#queue_event{reason = binary_to_atom(Reason)},
    ActionID);
queue_event([_Field|T], Record, ActionID) ->
...
agent_call_record([<<"Priority: ", Priority/binary>>|T], Record, ActionID) ->
  agent_call_record(T, Record#agent_call{
    ...
    ActionID);
agent_call_record([_Field|T], Record, ActionID) ->
...
log_record([_Field|T], Record, ActionID) ->
...
registry_record([<<"Status: ", Status/binary>>|T], Record, ActionID) ->
  registry_record(T, Record#registry{status = binary_to_atom(Status)},
    ActionID);
registry_record([_Field|T], Record, ActionID) ->
...
hold_record([_Field|T], Record, ActionID) ->
...
dnd_state_record([<<"Status: ", Status/binary>>|T], Record, ActionID) ->
  dnd_state_record(T, Record#dnd_state{status = binary_to_atom(Status)},
    ActionID);
dnd_state_record([_Field|T], Record, ActionID) ->
...
alarm_record([<<"Channel: ", Channel/binary>>|T], Record, ActionID) ->
  alarm_record(T, Record#alarm{channel = binary_to_integer(Channel)},
    ActionID);
alarm_record([_Field|T], Record, ActionID) ->
...
rename_record([<<"Privilege: ", Priv/binary>>|T], Record, ActionID) ->
  rename_record(T, Record#rename{privilege = privileges_list(Priv)},
    ActionID);
rename_record([<<"Channel: ", Channel/binary>>|T], Record, ActionID) ->
  rename_record(T, Record#rename{channel = binary_to_list(Channel)},
    ActionID);
rename_record([<<"Newname: ", Newname/binary>>|T], Record, ActionID) ->
  rename_record(T, Record#rename{newname = binary_to_list(Newname)},
    ActionID);
rename_record([<<"Uniqueid: ", Uniqueid/binary>>|T], Record, ActionID) ->
  rename_record(T, Record#rename{unique_id = binary_to_list(Uniqueid)},
    ActionID);
rename_record([_Field|T], Record, ActionID) ->
...
shutdown_record([_Field|T], Record, ActionID) ->
...
agent_event_record([_Field|T], Record, ActionID) ->
...
ast_state_record([<<"Uniqueid: ", Uniqueid/binary>>|T], Record, ActionID) ->
  ast_state_record(T, Record#ast_state{unique_id = binary_to_list(Uniqueid)},
    ActionID);
ast_state_record([_Field|T], Record, ActionID) ->
...
exten_record([<<"Uniqueid: ", Uniqueid/binary>>|T], Record, ActionID) ->
  exten_record(T, Record#exten{unique_id = binary_to_list(Uniqueid)},
    ActionID);
exten_record([_Field|T], Record, ActionID) ->
...
channel_record([<<"CallerIDNum: ", CallerIDNum/binary>>|T], Record, ActionID) ->
  ...
  normalize_number(binary_to_list(CallerIDNum))}, ActionID);
...
channel_record([<<"Exten: ", Exten/binary>>|T], Record, ActionID) ->
  channel_record(T, Record#channel{exten = binary_to_list(Exten)}, ActionID);
channel_record([_Field|T], Record, ActionID) ->
...
link_record([_Field|T], Record, ActionID) ->
...
hangup_record([_Field|T], Record, ActionID) ->
...
caller_id_record([<<"CallerIDNum: ", CallerIDNum/binary>>|T], Record, ActionID) ->
  caller_id_record(T, Record#caller_id{caller_id = normalize_number_callar(binary_to_list(CallerIDNum), Record)}, ActionID);
...
caller_id_record([_Field|T], Record, ActionID) ->
...
reload_record([_Field|T], Record, ActionID) ->
...
peer_status_record([_Field|T], Record, ActionID) ->
...
agent_record([<<"TalkingTo: ", TalkingTo/binary>>|T], Record, ActionID) ->
  agent_record(T, Record#agent{talking_to = binary_to_list(TalkingTo)},
    ActionID);
agent_record([_Field|T], Record, ActionID) ->
...
exten_state_record([_Field|T], Record, ActionID) ->
...
mailbox_count_record([_Field|T], Record, ActionID) ->
...
parked_call_record([<<"CallerIDName: ", CallerIDName/binary>>|T], Record, ActionID) ->
  ...
  binary_to_list(CallerIDName)}, ActionID);
parked_call_record([_Field|T], Record, ActionID) ->
...
queue_entry_record([<<"CallerID :", CallerID/binary>>|T], Record, ActionID) ->
  ...
  normalize_number(binary_to_list(CallerID))}, ActionID);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
queue_entry_record([<<"Uniqueid: ", Uniqueid/binary>>|T], Record, ActionID) ->
  queue_entry_record(T, Record#queue_entry{unique_id = binary_to_list(Uniqueid)},
    ActionID);
...
queue_entry_record([<<"Count: ", Count/binary>>|T], Record, ActionID) ->
  queue_entry_record(T, Record#queue_entry{count = binary_to_integer(Count)},
    ActionID);
queue_entry_record([_Field|T], Record, ActionID) ->
...
queue_member_record([_Field|T], Record, ActionID) ->
...
queue_params_record([_Field|T], Record, ActionID) ->
...
sip_peer_record([_Field|T], Record, ActionID) ->
...
sip_peer_details([_Field|T], Record, ActionID) ->
...
channel_status_record([_Field|T], Record, ActionID) ->
...
zap_channel_record([_Field|T], Record, ActionID) ->
...
%%% ============================================================================
parse_sip_number(<<"SIP/", Number:3/binary, _/binary>>, _Default) ->
  binary_to_list(Number);
parse_sip_number(_Num, Default) ->
  Default.

parse_phones(#call{subevent = 'Begin', caller_id_num = CallerIDNum, dialstring = Dialstring, channel = Channel} = Record) ->
  FromNumber = case string:str(CallerIDNum, "/") of
                 0 ->
                   case CallerIDNum of
                     "555200" ->
                       parse_sip_number(list_to_binary(Channel), CallerIDNum);
                     _ ->
                       CallerIDNum
                   end;
                 Pos1 ->
                   string:substr(CallerIDNum, Pos1 + 1)
               end,
  ToNumber = case string:str(Dialstring, "/") of
               0 ->
                 case Dialstring of
                   "555200" ->
                     parse_sip_number(list_to_binary(Channel), Dialstring);
                   _ ->
                     Dialstring
                 end;
               Pos2 ->
                 string:substr(Dialstring, Pos2 + 1)
             end,
  Record#call{from_number = normalize_number(FromNumber, Record#call.unique_id), to_number = normalize_number(ToNumber)};
parse_phones(Record) ->
  %io:format("Parse phones Not match~p~n",[Record]),
  Record.

normalize_number(Number) ->
  normalize_number(Number, 0).

normalize_number(Number, _Channel) ->
  try list_to_integer(Number) of
    %% for invalid number x and xx
    Num when Num < 100 ->
      0;
    %% for number xxx
    Num when Num < 1000 ->
      Num;
    %% for invalid number xxxx and xxxxx
    Num when Num < 100000 ->
      0;
    %% for number xx-xx-xx Region numbers
    Num when Num < 1000000 ->
      %% add 4 number city code
      0;
    %% for number xxx-xx-xx
    Num when Num < 10000000 ->
      %% add 3 number city code
      0;
    %% for number xx-xx-xx
    Num when Num < 1000000 ->
      380432000000 + Num;
    %% for number (0xx)xxx-xx-xx
    Num when Num < 1000000000 ->
      380000000000 + Num;
    %% for number 8(0xx)xxx-xx-xx
    Num when Num < 81000000000 ->
      300000000000 + Num;
    Num when Num < 381000000000 ->
      Num;
    _ ->
      0
  catch
...
normalize_number_callar(Number, Channel)->
%%   log4erl:debug(parce_number, "Input number: ~p ~n", [Number]),
  case Channel of
       0->"";
      _Value->ok
%%       log4erl:debug(parce_number, "Parce number channel: ~p ~n", [Channel])
  end,


  Result = try list_to_integer(Number) of
    %% for invalid number x and xx
    Num when Num < 100 ->
      0;
    %% for number xxx
...
%%   log4erl:debug(parce_number, "Parse result: ~p ~n", [Result]),
  Result.
...
cid_calling_pres(Binary) ->
  String = binary_to_list(Binary),
  {match, [{Start0, Len}|_]} = re:run(String, "(-)?[0-9]+"),
  Start = Start0 + 1,
...
get_local_code(3, Record)->
  _A=#res_tmp.code,
  Res1 = add_value(get_mach(Record#caller_id.channel, "datakiev"), 380440000000, 0),
  Res2 = add_value(get_mach(Record#caller_id.channel, "vn_555200"), 380432000000, Res1),
  Res3 = add_value(get_mach(Record#caller_id.channel, "it_443620753"), 380440000000, Res2),
  Res4 = add_value(get_mach(Record#caller_id.channel, "it_443621921"), 380440000000, Res3),
  Res5 = add_value(get_mach(Record#caller_id.channel, "it_443623580"), 380440000000, Res4),
  Res6 = add_value(get_mach(Record#caller_id.channel, "it_443623731"), 380440000000, Res5),
  Res7 = add_value(get_mach(Record#caller_id.channel, "it_443625608"), 380440000000, Res6),
  Res8 = add_value(get_mach(Record#caller_id.channel, "it_312444756"), 380312000000, Res7),
  Res9 = add_value(get_mach(Record#caller_id.channel, "it_567675423"), 380560000000, Res8),
  Res10 = add_value(get_mach(Record#caller_id.channel, "it_622138203"), 380620000000, Res9),
  Res11 = add_value(get_mach(Record#caller_id.channel, "it_622138204"), 380620000000, Res10),
  Res12 = add_value(get_mach(Record#caller_id.channel, "it_412469365"), 380412000000, Res11),
  Res13 = add_value(get_mach(Record#caller_id.channel, "it_617072348"), 380610000000, Res12),
  Res14 = add_value(get_mach(Record#caller_id.channel, "it_342543637"), 380342000000, Res13),
  Res15 = add_value(get_mach(Record#caller_id.channel, "it_522276458"), 380522000000, Res14),
  Res16 = add_value(get_mach(Record#caller_id.channel, "it_536703017"), 380536600000, Res15),
  Res17 = add_value(get_mach(Record#caller_id.channel, "it_642722873"), 380642000000, Res16),
  Res18 = add_value(get_mach(Record#caller_id.channel, "it_332290819"), 380332200000, Res17),
  Res19 = add_value(get_mach(Record#caller_id.channel, "it_322532384"), 380320000000, Res18),
  Res20 = add_value(get_mach(Record#caller_id.channel, "it_322532387"), 380320000000, Res19),
  Res21 = add_value(get_mach(Record#caller_id.channel, "it_512719130"), 380512000000, Res20),
  Res22 = add_value(get_mach(Record#caller_id.channel, "it_487043450"), 380480000000, Res21),
  Res23 = add_value(get_mach(Record#caller_id.channel, "it_532651067"), 380532000000, Res22),
  Res24 = add_value(get_mach(Record#caller_id.channel, "it_362400487"), 380362000000, Res23),
  Res25 = add_value(get_mach(Record#caller_id.channel, "it_692945241"), 380692000000, Res24),
  Res26 = add_value(get_mach(Record#caller_id.channel, "it_652709123"), 380652000000, Res25),
  Res27 = add_value(get_mach(Record#caller_id.channel, "it_652709124"), 380652000000, Res26),
  Res28 = add_value(get_mach(Record#caller_id.channel, "it_542790951"), 380542000000, Res27),
  Res29 = add_value(get_mach(Record#caller_id.channel, "it_352420481"), 380352000000, Res28),
  Res30 = add_value(get_mach(Record#caller_id.channel, "it_577800314"), 380570000000, Res29),
  Res31 = add_value(get_mach(Record#caller_id.channel, "it_552391550"), 380552000000, Res30),
  Res32 = add_value(get_mach(Record#caller_id.channel, "it_382705154"), 380382000000, Res31),
  Res33 = add_value(get_mach(Record#caller_id.channel, "it_472566741"), 380472000000, Res32),
  Res34 = add_value(get_mach(Record#caller_id.channel, "it_372903293"), 380372000000, Res33),
  Res35 = add_value(get_mach(Record#caller_id.channel, "it_462920270"), 380462000000, Res34),
  Res35.

get_mach(Search, Pattern)->
  case re:run(Search, Pattern) of {match,_Mach}->{value, ok};_->{empty, 0} end.
add_value(Res, OkValue, TmpValue)->
  case Res of
    {value, ok}->OkValue;
    {_,_}->TmpValue
  end.
...
%% ast_parser:get_local_code(3, Record).
%% Record = {caller_id,[all,call],  "SIP/vn_555200-0001deaa",undefined,  undefined,undefined,undefined}.
%% Record = {caller_id,[all,call],  "SIP/it_567675423-0001deaa",undefined,  undefined,undefined,undefined}.
%% "555777", {caller_id,[all,call],  "SIP/vn_555200-0001deaa",undefined,  undefined,undefined,undefined}
%% ast_parser:normalize_number_callar("555777", {caller_id,[all,call],  "SIP/vn_555200-0001deaa",undefined,  undefined,undefined,undefined}).
%% ast_parser:normalize_number_callar("555777", {caller_id,[all,call],  "SIP/it_472566741-0001deaa",undefined,  undefined,undefined,undefined}).
%% ast_parser:normalize_number_callar("379787671114", {caller_id,[all,call],  "SIP/it_472566741-0001deaa",undefined,  undefined,undefined,undefined}).
